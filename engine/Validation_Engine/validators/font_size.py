...
def _normalize(s: str) -> str:
    """Strips all whitespace before comparing — OCR/regex extraction can
    normalize spacing differently than the original detected text span
    (e.g. entity '110g' vs field text '110 g'), which would otherwise make
    an exact substring check fail even though they're the same value."""
...
def _find_field_for(text_fragment: str | None, fields: list[CalibratedField]) -> CalibratedField | None:
    if not text_fragment:
        return None
    normalized_fragment = _normalize(text_fragment)
    for f in fields:
        if normalized_fragment in _normalize(f.text):
            return f
    return None
